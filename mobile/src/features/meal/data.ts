...
export interface MealCoachLine {
  trigger: string;
  lead: string;
  emph: string;
  tail: string;
...
export const SAMPLE_PARSES: SampleParse[] = [
  {
    id: 'breakfast',
...
export function pickSampleForInput(text: string | null | undefined): SampleParse {
...
export function totalMeal(items: MealItem[]): MealTotals {
...
export function rescaleItem(item: MealItem, newGrams: number): MealItem {
...
export const DAY_STATES: Record<string, DayState> = {
  empty: {
...
export function dayTotals(meals: LoggedMeal[]): MealTotals {
  return meals.reduce((acc, m) => ({
    kcal: acc.kcal + (m.totals as MealTotals).kcal,
    p:    acc.p    + (m.totals as MealTotals).p,
    c:    acc.c    + (m.totals as MealTotals).c,
    f:    acc.f    + (m.totals as MealTotals).f,
  }), { kcal: 0, p: 0, c: 0, f: 0 });
}

// Coach lines, picked after a save based on protein progress
export const MEAL_COACH_LINES: MealCoachLine[] = [
  { trigger: 'low_protein',
...
export function pickMealCoach(
  { meals, lastMeal, totals }: { meals: LoggedMeal[]; lastMeal?: any; totals: MealTotals }
): MealCoachLine | undefined {
...
// ── Next-meal suggestion (ported from meal-today.jsx helpers) ───────────

export function suggestNextMeal(meals: LoggedMeal[], now?: string): MealSuggestion {
