...
//
// The 2026-08-14 decision put provisioner at cmd/provisioner inside the api
// module rather than in a module of its own, and named exactly what that gives
// up: the compiler stops being the thing that proves this component cannot
// reach the database. Two checks replace it — step-6's grep rule on the admin
// key's variable, and this one.
//
// It is transitive on purpose. `depguard` and a reading of the import block
// both see only the direct edge, and the shortest way in is not a direct edge:
// one import of internal/platform/database or internal/identity brings pgx, the
// pools, and the seam that sets a caller's claims. The component that holds the
// admin key would then be one line from the schema the key exists to be kept
// away from.
