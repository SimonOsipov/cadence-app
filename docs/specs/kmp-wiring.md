...
> AndroidKeyStore, ключ не выдаётся, `write` отказывает, и `VaultSettings` держит запись у себя
> с `writable = false` (`Unavailable` тут не возникает: файла ещё нет, чтение отвечает `Absent`). То есть такое утверждение падает на **корректной**
...
>
> **Третий раунд: адрес GoTrue не был пришпилен ничем, и предыдущий абзац это преувеличивал.**
> Утверждение «поменяй адреса местами — тест убивает поимённо» верно лишь для перестановки: она
> ломала API-сторону. Мутация **одного** адреса GoTrue выживала, и вместе с ней выживала
> `cadenceAuth(url = api, …)` — то есть пароль, уходящий на адрес API, оставлял весь гейт зелёным.
> Причина не в тесте: `supabase-kt` строит собственный Ktor-клиент, и мок-движок его не видел.
> В артефакте есть `SupabaseClientBuilder.setHttpEngine` (проверено `javap`), поэтому `cadenceAuth`
> принимает движок, `CadenceDebug` отдаёт ему свой, и вход теперь идёт через тот же мок. Убийца
> мутации — `eachCallGoesToItsOwnService`.
>
> Тот же шов закрыл две дыры даром. `/healthz` без учётки проверялся на клиенте, собранном в
> тесте, а не на том, что строит `CadenceDebug`, — мутация «дать `raw` плагин аутентификации»
> проходила; теперь её убивает `healthCarriesNoCredentialAndTheApiCallDoes`. И у `SignIn.Refused`
> появились два свидетеля: `aRefusedSignInReportsAClassAndNotAMessage` пришпиливает
> `AuthRestException` **по имени**, а `aRefusalCarryingASessionDoesNotPutItOnTheScreen` спрашивает
> саму опасность — отказ с телом-токеном не должен положить токен на экран.
>
> Первая редакция второго утверждала «в тексте нет пробела», и это убивало мутацию по случайности
> фикстуры: `respondError` даёт тело `Bad Request`, вендор не разбирает его как JSON и подставляет
> «Unknown error» — пробел оттуда. Настоящий отказ GoTrue, `invalid_credentials`, пробела не имеет,
> и мутация пережила бы его молча. Ревью назвало класс `BadRequestRestException`; замер говорит
> `AuthRestException` — поэтому пришпилен измеренный, а не названный.
>
