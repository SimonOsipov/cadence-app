...
**Из пяти реально остаётся четыре.** `weight_series` — единственный срез, который
сервер и правда шлёт как `null`. Остальные списки чтений (`week_protocol`,
`RowBody.times`, `days`, `occurrences`) публиковались `["array","null"]` не по
решению, а потому что huma так типизирует Go-срез **по умолчанию** — и это
снимается тегом `nullable:"false"`, что в этом репозитории уже сделано дважды
(`identity/roster.go:113`, `identity/directory.go:93`). Найдено ревью шага 12 и
**починено, а не записано**: все четыре сервер строит через `make` до заполнения,
теперь это сказано и в контракте.
...
**Замерено:** `api/internal/protocol/routes.go:517-530` против
`TodayRepository.kt:37-47`. Не расхождение: `RowBody.compound` — у нас `null` для
...
говорит,
что это «at most wrong by one vial».
