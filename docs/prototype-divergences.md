...
- **The photo upload** — server half paid 2026-08-24. The API mints a key and
  hands back a signed PUT, and the dose write takes `photo_path`. What remains is
  the KMP half: `DoseDraft` still carries `photoAttached` as a boolean and the
  write drops it, so the wizard has to upload before it saves and carry a path.
...
## ~~Питание: футер мастера прокручивается~~ — оплачено 2026-08-14
...
**Что делали мы до 14.08:** футер был последним ребёнком прокручиваемой колонки,
поэтому при разобранном приёме из пяти позиций уходил ниже сгиба — главное
действие экрана приходилось искать прокруткой. Решения такого никто не принимал;
нашло ревью шага 15.

**Что делаем теперь:** `Box` с прокруткой под фиксированной панелью, колонка
кончается зазором `FOOTER_CLEARANCE`, панель красит свой фон — иначе позиции
читались бы сквозь неё. `theSaveFooterStaysOnScreenOnceTheParseFillsTheList`
меряет низ панели при экране 375×667 по нескрытой геометрии (`positionInRoot +
size`): на клипованных `boundsInRoot` такая проверка не может упасть в принципе.
Второй тест держит высоту панели внутри зазора.
...
**Что делает API:** `GET /v1/me/today` отдаёт по ним `null`. Первые три — потому
что контекст питания не построен; `weight_series` — по причине, которая с тех пор
переменилась (см. ниже); `vial_doses_left` отсутствует, когда у пациента нет
открытого флакона этого препарата.

**Половина причины отпала 2026-08-31.** Контекст измерений построен — таблица, обе
ручки чтения, ручная запись и удаление, — и `weight_kg` с `weight_series` по-прежнему
`null`, но уже по другой причине: `today` его не читает. Встречная нужда припаркована
(`protocol/imports_test.go:31-35`): когда M6 её раскроет, `protocol` объявит четвёртый
потребительский интерфейс, а не развернёт импорт. Ряды веса тем временем отвечает
`GET /v1/me/trends`.

**Из списков остаётся один.** `weight_series` — единственный срез, который сервер и
правда шлёт как `null`. Остальные пять списков этого блока (`week_protocol`,
`RowBody.times`, `days`, `occurrences` и `item_ids` ответа на выписку)
публиковались `["array","null"]` не по решению, а потому что huma так типизирует
Go-срез **по умолчанию** — и это снимается тегом `nullable:"false"`, что в этом
репозитории уже сделано дважды (`identity/roster.go:113`,
`identity/directory.go:93`). Найдено ревью шага 12 и **починено, а не записано**:
все пять сервер строит непустыми до заполнения, теперь это сказано и в контракте, и
закреплено тестами, которые падают, если конструктор заменить на nil. Пять
непустых полей отвечает и остаток KMP-типов выше: те четыре остаются, потому что
там значения действительно нет.
...
решение**, которого сервер не принимал. Это верно только для тех четырёх и для
`weight_series` — то есть там, где значения действительно нет; для списков
лекарство было серверным и уже применено.

**Одно поле в обратную сторону:** `Compound.code` в KMP не-nullable
(`Protocol.kt:12`), а `CompoundBody` его не несёт вовсе — колоночный грант
пациента его намеренно не содержит, ключ сида остаётся на сервере. Это записано
решением в «Открытых вопросах» спеки, а не здесь.

**Замерено:** `api/internal/protocol/routes.go`, `TodayBody`, против
`TodayRepository.kt:37-47` — строками, а не номерами: номера в этом файле уже
уехали дважды за один шаг. Не расхождение: `RowBody.compound` — у нас `null` для
...
**Почему он остаётся — и это слабее, чем я записал сначала:** `dosing.Resolve`
вызывается ровно из двух мест, `Log` (то есть транспорт) и собственные модульные
тесты пакета. Никакого внепланового вызывающего нет: `cmd/seed` пакет `dosing` не
импортирует вовсе и строит `protocol.Draft`, а не `dosing.Draft` — два разных типа,
которые я спутал. То есть исход опубликован клиентам ради ветки, до которой
добирается только `log_test.go`.

**Чем это платится:** ветка клиента, которая никогда не выполнится. Решать в фазе
4, и на верных основаниях: снять исход из контракта, оставив его внутренним, или
оставить и написать в описании операции, что по HTTP он недостижим.
...
**Что делает API:** `OpenVialFor` (`internal/inventory/vials.go`) разрешает
...
подсказка велела купить, кандидатов два — и арифметика замолчала бы ровно тогда,
когда запас приехал. Отложенный не участвует ни в одном слое.

**Что делает KMP:** пикер есть с 04.08 — `DoseSteps.kt:204-216`: он ничего не
рисует при одном флаконе (`if (vials.size < 2) return`, `:210`) и кладёт выбранный
**в черновик**, а не только на экран. Резервный путь мока (`CadenceMocks.kt:511`) берёт самый полный и сам
...
## API: «отложен» — это дата, а не статус

**Что делает прототип:** ряд «Перенести в запас» в шторке флакона вызывает
`onClose`. Действия за ним нет, состояния тоже — см. «The vial sheet's two unbuilt
actions say so» выше.

**Что делает API:** `PUT /v1/me/vials/{vialId}/held-back` ставит и снимает
`held_back_at` — **дату, а не флаг**. Флакон остаётся своим и неутилизированным и
просто не участвует ни в одном решении, которое сервер принимает за пациента: он
не разрешается под дозу (`OpenVialFor`), не считается запечатанным дублем,
гасящим подсказку, и не считается запасом, кормящим её (`ReorderHintFor`), и не
даёт остаток на карточке «Сегодня» (`SupplyFor`). Утилизация снимает дату:
утилизированный флакон не бывает ещё и отложенным.

**Чего сервер не отвечает:** что происходит с уже списанными дозами при возврате
в запас — тот самый вопрос, из-за которого ряд прототипа остался заглушкой.
Сервер ничего не переносит, он помечает.

**Почему это здесь:** правило появилось на сервере раньше экрана, и фаза 4
портирует ряд под него, а не наоборот.
...
(`INJ_DOW` — воскресенье). А `today/TodayScreen.tsx:100-132` строит `protocolRows`
ровно из **трёх** элементов — семаглутид, BPC-157, глицин, — которые рисуются под
заголовком «Протокол этой недели» (`:551`, `.map` на `:566`), и взвешивания среди
них нет. («Воскресенье, утро · 4-я неделя» — это фиксированная строка даты над
героем, `:140`; я приписал её строкам, унаследовав ошибку из записи шага 11.)
...
## ~~API: подсказка докупки молчит на титрующемся курсе~~ — оплачено, 2026-08-29

**Что требует нота компонента:** `inventory.md`, инвариант 3 — «недели запаса
считаются от текущей **фазы титрации**, а не от справочной дозы; титрующийся
пациент иначе получит неверный ответ». План партнёра требует того же в приёмке:
доказать правила докупки «across a titration boundary»
(`build-plan-0-to-mvp-v1.0.md:499`).

**Что делает система:** `ReorderHintFor` считает `недели = остаток доз / доз в
неделю`, и доза в арифметику не входит нигде. Остаток — `total_doses` минус число
событий, то есть **число введений**, вписанное клиникой один раз.

**Замерено.** Флакон, засчитанный как 8 введений на дозе 0,25 мг, — это 2 мг
вещества. На курсе 0,25 → 0,5 → 1,0 мг:

| фаза | реально введений | что отвечает система |
|---|---|---|
| 0,25 мг | 8 | молчит — верно, 8 > порога 4 |
| 0,5 мг | **4** | **молчит** — порог пройден |
| 1,0 мг | **2** | **молчит** — порог пройден вдвое |

`total_doses` при смене фазы не меняется, поэтому счёт продолжает отдавать восемь
недель, и подсказка не выдаётся **вовсе**. Это тишина вместо предупреждения, а не
позднее предупреждение. Тем же путём врут `VialStatus.LOW` (порог «меньше
четверти» — от `total_doses`) и остаток флакона на карточке «Сегодня».
...
(она уже есть в `dose_events` с шага 8), остаток введений выводится делением на
дозу текущей фазы.

**Оплачено, 2026-08-29.** Предложение принято и сделано: 000021 дал флакону
`total_amount` и `amount_unit`, 000022 убрал `total_doses`, и остаток — вычитание
вещества (`RemainingAmount`). `RemainingDoses` делит его на дозу **текущей фазы**,
поэтому таблица выше теперь читается по колонке «реально введений»: 8 → 4 → 2, и
...
## API: «Сон /100» — у мока это хранимый ряд, у сервера пустая и невписуемая метрика

**Что делает KMP:** `MockSeed.buildMeasurements` выписывает `Metric.SLEEP` ежедневным
рядом от 72 до 86 с источником `MeasurementSource.HEALTH_KIT`
(`MockSeed.kt:377-379`), и комментарий рядом говорит почему: у `MeasurementSource` нет
значения «derived», а импорт — ближайшая правда для метрики, которую считает сервер.
`MetricMeta` при этом держит её `DEVICE_ONLY` (`MetricMeta.kt:126`), так что руками её
не предлагают и там.

**Что делает API:** метрика в наборе из восьми, обе ручки чтения её отвечают — и ряд
её пуст у всякого пациента. Импортёра нет, формулы нет («a derived score computed by
the API … v1: duration-based formula, constants module»,
`source/architecture-overview-v1.1.md:182`), сид не пишет ни одной её строки. Записать
её тоже нечем: `WritableMetrics()` — семь метрик без сна, `Record` отвечает
`ErrMetricNotWritable`, а `enum` тела запроса её не содержит. Отказ здесь именно
API-шный: таблица `metric IN (…)` сон допускает, и грант пациента на `INSERT` его не
различает — политика смотрит только на `patient_id`. Служебная роль вписать его сможет,
когда будет чему.

**Чем это платится:** портированный экран трендов рисует карточку «Сон» с рядом от
мока и пустую — с живого API, и разница видна ровно в момент подключения фазы 4. Пустой
ряд при этом ответ, а не ошибка: метрика не выпадает из восьми, потому что «не измерено»
— это тоже то, что пациент должен уметь узнать.

**Что закрывает расхождение:** импортёр M5 и формула рядом с ним, в этом же модуле.

## API: `MetricMeta` и пороги — контракт возит истину, обе поверхности держат копии

**Что требует канон:** «Thresholds (HRV ≥58 · RHR ≤60 · Sleep ≥75) move from client
code to one constants module in the API so both surfaces agree»
(`source/architecture-overview-v1.1.md:182`) — он же инвариант 1 ноты `measurements.md`.

**Что делает API:** модуль появился — `measurements.Meta`. Единица, направление и порог
едут в ответе обеих ручек чтения (`MetricMetaBody`); у пяти метрик из восьми порога нет,
и он отсутствует, а не равен нулю: все пять смотрят «вниз», и нулевой границы не прошло
бы ни одно показание. Клинически значимая половина модуля опубликована.

**Что делают поверхности:** ни одна его пока не читает. KMP держит свой
`MetricMeta.kt` — восемь строк с единицей, направлением, подписью, числом знаков и
акцентом; дашборд держит свои пороги прямо в фикстуре
(`web/src/data/fixtures/overview.ts`) плюс собственные русские подписи и единицы у
каждого биомаркера. Замерено: 25 пациентов фикстуры, у каждого те же три строки — `hrv
min 58`, `rhr max 60`, `sleep min 75`, — то есть ровно числа канона, выписанные в третий
раз.

**Почему это здесь, а не в долге:** копии не «остались случайно» — фикстура дашборда
и мок KMP кормят экраны до фазы 4, и убрать их раньше проводов значит погасить оба
экрана. Инвариант 1 фича **не закрывает**: она даёт модулю дом и провод, а сходятся
поверхности тогда, когда начинают читать ответ. До этого три числа живут в трёх местах,
и разойтись им ничто не мешает.

**Что остаётся на клиенте по решению, а не по недоделке:** подпись, число знаков после
запятой и акцент. Это оформление, и `MetricMetaBody` их не возит намеренно.

## API: направление бёдер и груди названо полем, которого у прототипа нет

**Что даёт §03:** явного направления — «куда метрике двигаться, чтобы пациенту
становилось лучше» — нет ни у одной из восьми. У трёх с порогом его задаёт сторона
сравнения: «HRV ≥58 · RHR ≤60 · Sleep ≥75»
(`source/architecture-overview-v1.1.md:182`), и `meta.go` это проговаривает.

**Что даёт прототип:** полем — шесть из восьми.
`mobile/src/features/trends/data.ts` держит `direction` у семи своих метрик, и шесть из
них наши: вес `down`, HRV `up`, пульс покоя `down`, сон `up`, процент жира `down`,
талия `down`. Седьмая — `thigh`, которой в §03 нет. **Бёдер и груди в этом модуле нет
вовсе.** Они живут на экране «Тело», и направление у них там **тоже есть — но не
полем, а общим правилом**: `delta()` (`features/body/data.ts:88-93`) возвращает
`down = d <= 0` под комментарием «for these metrics down = progress», и экран красит
этим `down` каждую строку `measureRows = ['waist','hip','chest','bmi']`
(`BodyScreen.tsx:723`, цвет на `:561` и `:922`).

**Что делает API:** `Meta` отвечает `DirectionDown` обеим (`meta.go:30-33`). Это то же
`down`, что рисует экран «Тело», превращённое в поле на метрику. Тем же путём его уже
прошёл KMP: `MetricMeta.kt:74-76` выводит направление обеих из талии, и только грудь
подпёрта сеяной историей того же экрана (`MetricMeta.kt:175-176`, `SEED_HIST.chest` идёт
112 → 105).

**В чём тогда расхождение:** не в значении, а в его форме и охвате. Направление этих двух
у прототипа — одно правило на весь экран «Тело», у нас — поле на каждую из восьми метрик,
и его можно поменять по одной. Правило прототипа при этом покрывает и `bmi`, которого
среди восьми §03 нет, а нашего поля не хватает там, где «вниз» неверно.

**Почему сервер повторил клиента, а не решил заново:** направление — это сторона порога
и знак «прогресса» на карточке. Два ответа на него дали бы стрелку вверх на одной
поверхности и вниз на другой для одной и той же строки таблицы.

**Что это стоит:** ровно одно допущение — что обхват груди у пациента на протоколе
снижения веса должен уменьшаться. Для пациента на наборе массы оно неверно, и тогда это
поле модуля констант, который §03 обещает, а не строчка в `meta.go`.

## API: сервер принимает семь метрик руками, клиент предлагает пять

**Что делает прототип:** `mobile/src/features/body/data.ts` помечает `editable: true`
пять метрик — вес, процент жира, талию, бёдра, грудь, — и это ровно те, что попадают в
шторку «Добавить замер». HRV, пульса покоя и сна на этом экране нет.

**Что делает KMP:** ровно те же пять. `MetricEntry.BY_HAND` стоит у пяти строк
`MetricMeta.kt`, `DEVICE_ONLY` — у HRV, пульса покоя и сна, и
`theMetricsThePatientTypesInAreThePrototypesEditableOnes` держит этот набор поимённо.

**Что делает API:** семь. `WritableMetrics()` — те же восемь метрик без сна, и
`TestTheOneMetricNoPatientTypesIsTheSleepScore` называет исключённую метрику, а не
считает длину набора. HRV и пульс покоя записываются руками, потому что это числа,
которые часы **показывают**: пациент без импорта читает их с циферблата и вводит, и
инвариант 6 ноты `measurements.md` — «ручной ввод остаётся первоклассным путём везде» —
требует именно этого. Пять прототипа — это состав одной шторки, а не набор API.

**Чем это платится:** двумя метриками, которые сервер примет, а ни один экран сегодня не
предлагает ввести. Это недостача интерфейса, а не лишний грант: обратное — пять на
сервере — сделало бы ручной ввод HRV невозможным для пациента без часов, а импорта нет
до M5.

## API: `TrendsOverview.range` не допускает пустого, а сервер его отвечает

**Что делает KMP:** `TrendWindow.rangeOn` отвечает `TrendRange?`, и `null` — это окно,
которое не покрывает ни одного дня. Сам `TrendRange` пустого не бывает: его `init`
требует `from <= through`, то есть минимум один день. Но поле, в которое ответ ложится,
— `TrendsOverview.range` — объявлено непустым `TrendRange`. Отсутствие, которое умеет
выразить функция, тип экрана выразить не умеет.

**Что делает API:** `range` в ответе обеих ручек чтения — nullable, и `null` там в двух
случаях: у пациента нет курса вовсе, и курс ещё не начался (`window.go:54-61`). Ни один
из трёх фиксированных отрезков пустым не бывает — пустым бывает только окно цикла,
потому что оно геометрия курса, а не длина. Из этих двух случаев `rangeOn` умеет
выразить **один**: `ProtocolPlan.protocol` не nullable, так что «курса нет вовсе» KMP на
этом входе не выражает, а `null` у него — только «курс ещё не начался»
(`TrendWindow.kt:105-106`).

**Почему сервер не подставил день:** у мока подстановка защитима и его KDoc так её и
называет — «the honest answer», — потому что курс у мока есть всегда и единственный его
`null` означает «курс ещё не начался». У сервера случай другой: пациент без курса вовсе,
и окно цикла шириной в сегодня сказало бы про него, что курс есть и идёт первый день.
Пустое окно — ответ, а не ошибка; `Span` в `read.go` записывает это рядом с полем.

**Что чинить в фазе 4:** `TrendsOverview.range` становится `TrendRange?`, и экран
получает состояние «окна нет» — которого у мока не бывает, потому что его `rangeFor`
подставляет `TrendRange(currentDate(), currentDate())` там, где `rangeOn` ответил `null`
(`CadenceMocks.kt:338-340`). До тех пор клиент, собранный против KMP-типа и посаженный
на этот API, разбирает `null` в непустое поле.
