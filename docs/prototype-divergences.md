...
запускается на заглушках.~~ **Закрыто.** Это оказалось не расхождением
оформления, а тупиком: на вкладку ничего не кладётся сверху, поэтому и жеста
«назад» нет — выйти можно было только убив приложение. Оба экрана рисуют
`CadenceTabBar` (`VialsScreen.tsx:295`, `TrendsScreen.tsx:543` в прототипе);
сторож `thePortedTabsKeepTheBarAndLetYouLeave` заходит на обе вкладки **с
данными** — на пустых заглушка подставляется и бар рисует сама, и та же
проверка проходит, пока пациент заперт.
