 ...
 * (`mobile/src/features/meal/data.ts:79,93,107`), not the bare label a canned parse is
 * named after: `'lunch-bowl'`'s triggers are `bowl`/`рис`/`капуста`/`тахини`, not the
 * Cyrillic word `боул` itself, so a lone `"боул"` would score zero everywhere and fall
 * through to breakfast. The full placeholder carries `рис` (inside `рисом`) and `тахини`,
 * which is what actually wins the lunch parse under `pickSampleForInput`'s literal scoring
...
class MockMealParserTest {
    ...
    fun snackKeywordTvorogGivesTheSnackParse() =
        runTest {
            // meal/data.ts:107 — the snack sample's placeholder text.
    ...
    fun lunchKeywordBowlGivesTheLunchParse() =
        runTest {
            // meal/data.ts:93 — matches on рис/тахини, not on the Cyrillic word боул itself.
    ...
    fun tiedZeroScoresBreakToTheFirstEntryNotTheLastOne() =
        runTest {
            // Every trigger scores zero, so the winner comes entirely from the tie-break:
            // `score > bestScore`, seeded at -1, replaces only on a strictly higher score,
            // so breakfast (checked first) wins the all-zero tie. A `>=` mutant would keep
            // replacing through the list and land on snack, the last entry, instead.
    ...
    fun cannedParsesCarryThePrototypesPlaceholdersVerbatim() {
        // meal/data.ts:79,93,107 — ported whole so the step-5 example-cycling control has
        // real text to read rather than reconstructing it from the transcript.
        assertEquals(
            listOf(
    ...
    fun cannedParsesCarryThePrototypesTriggerWordsVerbatim() {
        // meal/data.ts:78,92,106 — the actual scoring input; a corrupted trigger silently
        // resolves real text ("chicken bowl", "yogurt") to the wrong canned meal without any
        // test noticing, since every other fixture here only exercises the winner's triggers.
        assertEquals(
            listOf(
                listOf(
                    "yogurt",
    ...
    fun aContestedScoreEndsWithTheHigherScoringParseNotTheFirstMatch() =
        runTest {
            // "йогурт" scores breakfast 1; "рис"/"тахини" score lunch-bowl 2; snack scores 0.
            // Counting, not existence, is what `pickSampleForInput` does (meal/data.ts:122-124):
            // one point per matching trigger, not one point for having any. An `any`-based
            // mutant would tie breakfast and lunch-bowl at 1 each and land on breakfast
            // (checked first) — asserting Обед here catches that. A `>=` tie-break mutant
            // doesn't change this result (2 still beats 1); that one is caught by
            // `tiedZeroScoresBreakToTheFirstEntryNotTheLastOne` above instead.
