...
// Shared with the integration file, which runs the same command against a real database.
const (
...
// recorder is a writer that reaches no database and remembers what it was asked to write.
...
func TestRunPassesTheAdministratorAndTheMigrationURLToTheWrite(t *testing.T) {
	...
	// Trimmed: the space around a typed name would otherwise be part of it everywhere it is displayed.
...
// The arms that must refuse, and what makes refusal worth asserting: a half-run on a mistyped identifier leaves a
// profile nobody can sign in as and an audit row saying it was deliberate.
func TestRunWritesNothingOnAnArgumentItRefuses(t *testing.T) {
	for name, args := range map[string][]string{
		...
		// pgx's parser accepts these separators and uuid_in does not — the reason recorded at database.IsUUIDShaped.
...
// Configuration first would answer a mistyped argument with a missing connection string — the wrong thing to fix.
...
// No falling through to libpq's default, which resolves to the local user and, in a container, to the superuser.
...
// Swallowing the write's failure exits zero on a clinic with no administrator, and the pipeline reports success.
