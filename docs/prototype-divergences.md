...
**Шестое поле, и оно шире прочих:** `week_protocol` (`routes.go:514`) — голый
`[]RowBody`, а huma типизирует **всякий** Go-срез как `["array","null"]` и держит
его обязательным. KMP объявляет `List<ProtocolRow>` не-nullable. На проводе туда
`null` не уходит никогда — `renderToday` всегда строит хотя бы пустой срез, — но
для клиента, сгенерированного из контракта, цена та же, что у пяти выше.

**И одно поле в обратную сторону:** `Compound.code` в KMP не-nullable
...
**Замерено:** `api/internal/protocol/routes.go:514-526` против
