...
>
> **SKL-01 и SKL-06 заменены локальным контуром, и вот что это покупает, а что нет.** Куплено:
> GoTrue, Postgres и хранилище объектов поднимаются `make dev-up`, и форма входа измерена против
> них. Не куплено: **живой `GET /v1/me`**. API отказывается стартовать без провизионера, а
> сервиса провизионера в `docker-compose.yml` нет — это SKL-06 в конкретной форме, а не в общей.
> Приёмочный пункт «экран показывает живой `GET /v1/me` с тестовым пользователем» остаётся
> невыполненным, и это единственный пункт шага, который остаётся.
>
...
> компиляции. Приёмка — греп артефакта, и она **в гейте**, а не разовым замером: обе сборки
> собираются и грепаются, маркер найден в `classes12.dex` debug-сборки девятнадцать раз и ни
> разу во всех трёх dex релизной, а мутация «сделать `implementation`» ловится по имени.
> **Названный должок:** iOS-половину этот греп не проверяет, потому что релизного iOS-артефакта
> не существует — `ios.sh` строит только Debug, и релизный путь принадлежит блоку выкатки.
> Спека это предвидела («acceptance includes a release build path that does not exist today»);
> для Android путь теперь есть, для Apple ещё нет.
>
...
>
> **Три вещи всплыли по ходу, каждая измерена, ни одна не была в шаге.** У модуля не было
...
> этой машины; путь теперь выводится из `xcode-select` и верен на любом хосте.
>
