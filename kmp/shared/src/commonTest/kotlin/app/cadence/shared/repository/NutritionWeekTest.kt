...
class NutritionWeekTest {
    /**
     * The guard `TrendRange` and `MetricMeta` set the precedent for: before this
     * `init` existed, "a week always carries seven days" was a comment on
     * `NutritionScreen.kt`'s `weekProteinAverage`, not something enforced — a
     * short or empty `days` list reached that division and threw
     * `IllegalArgumentException: Cannot round NaN value` instead of failing at
     * the boundary that actually built the malformed value.
     */
    @Test
