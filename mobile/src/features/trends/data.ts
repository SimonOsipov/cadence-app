...
export const TODAY_DAY = 84;

export interface ProtocolEvent {
  day: number;
  kind: 'start' | 'titration' | 'add';
  short: string;
  short_alt?: string;
  label: string;
}
...
export const DOSE_SPANS: DoseSpan[] = [
...
export type Timeframe = '7d' | '4w' | '3m' | 'cycle';

type SeriesMap = Record<Timeframe, number[]>;

export function makeWeight(): SeriesMap {
  ...
  const months: number[] = []; // last 28 days, ~daily granularity
  for (let i = 0; i < 28; i++) {
    const t = i / 27;
...
export function makeHRV(): SeriesMap {
...
export function makeRHR(): SeriesMap {
...
export function makeSleep(): SeriesMap {
...
export function makeBodyFat(): SeriesMap {
...
export function makeThigh(): SeriesMap {
...
export function makeWaist(): SeriesMap {
...
export type TrendSeries = Record<Timeframe, TimeframeSeries>;

// Day indexes parallel to each series (so the chart can map points → cycle day → annotation).
export function daysFor(seriesLength: number, span: number): number[] {
...
export function withDays(seriesMap: SeriesMap): TrendSeries {
  return {
...
export interface TrendRecentEntry {
  day: number;
  value: number;
  note: string;
}
...
export interface TrendBiomarker {
  ...
  accent: string;
  narrative: string;
  coach: { lead: string; emph: string; tail: string };
  correlations: TrendCorrelation[];
  recent: TrendRecentEntry[];
}

export const TREND_DATA: Record<string, TrendBiomarker> = {
  weight: {
    id: 'weight',
    label: 'Вес',
    eyebrow: 'Масса тела',
...
export const TREND_ORDER = ['weight', 'hrv', 'rhr', 'sleep', 'bodyfat', 'thigh', 'waist'];

// Helpers ───────────────────────────────────────────────────────────────

export function fmtValue(v: number, decimals: number): string {
...
export function seriesDelta(seriesAtTf: TimeframeSeries, decimals: number): SeriesDelta {
...
export function formatDeltaPill(seriesAtTf: TimeframeSeries, biom: TrendBiomarker): string {
...
export function isProgress(seriesAtTf: TimeframeSeries, biom: TrendBiomarker): boolean | null {
  const { diff } = seriesDelta(seriesAtTf, biom.decimals);
...
// Stat strip values
export function seriesStats(seriesAtTf: TimeframeSeries, biom: TrendBiomarker): { avg: string; min: string; max: string } {
  const v = seriesAtTf.values;
...
export function dayToFraction(day: number, seriesAtTf: TimeframeSeries): number | null {
...
export function normalizeForSpark(seriesAtTf: TimeframeSeries): number[] {
  const v = seriesAtTf.values;
