...
- [ ] **Ruleset применён**, а не подготовлен: права `admin` обещаны партнёром 29.07.2026, поэтому шаг доводится до конца здесь же
- [ ] Критерий проверяемый, а не декларативный: прямой push в `main` **фактически отклоняется**, и слияние без зелёных проверок невозможно — оба проверены попыткой
...
| **права admin** | BST-10 — обещаны партнёром 29.07.2026 | применение защиты `main` |
...
`CODEOWNERS` и файл ruleset с точным списком обязательных проверок кладутся в репозиторий и **применяются**: партнёр выдаёт права `admin` (BST-10), так что шаг больше не останавливается на «подготовлено». Приёмка — попытка прямого push в `main` и попытка слияния с красной проверкой; обе должны быть отклонены.
