...
> принимает движок, `CadenceDebug` отдаёт ему свой, и вход теперь идёт через тот же мок. Мутация
> падает поимённо.
>
...
> проходила; теперь падает. И у `SignIn.Refused` появился свидетель: на экран уходит имя класса
> исключения, а не его текст, потому что текст на этом пути может нести токен.
>
