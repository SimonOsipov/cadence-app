...
> [!deviation] 2026-08-16 — what the cycle suite decided that the step's paragraph does not say
> The step conforms: the whole sequence runs against one Postgres and one GoTrue that the harness brings up, every request goes over the surface `cmd/api` mounts with the token the provider minted for the person making it, and the residual property is asserted rather than described. What follows is what had to be decided inside it.
> 1. **The cycle begins with `cmd/bootstrap-admin`, not with a seeded administrator.** Step-4's deviation 1 fixed the operator's sequence — `POST /invite` on the provisioner, then the command with the identifier it answered — and the suite runs the half it can: the account is created at the provider by an admin call, the way `cmd/provisioner` creates one, and then the command as shipped — compiled to a binary *before* the invitation goes out and run after it, because the link is good for a minute and a cold build cache would spend it linking. The component itself is a `package main` and is not in the picture here; the command is run rather than imitated because a copy of its two statements would be a second implementation for the suite to measure instead of the one a deployment runs. It is also what gives the administrator a session at all: without an account at the provider there is nobody to sign in and create the first doctor.
> 2. **Three of the five refusal branches already had tests, and are re-measured here rather than duplicated or skipped.** A doctor on `/v1/providers` is step-6's, an account this clinic did not invite is step-5's, and the retry after the link was clicked is both; a patient on `/v1/patients` is a sixth the step does not list and the suite carries anyway, because it is the branch a real patient token reaches. What the second measurement adds is the road: those suites construct a principal and put it in the context, and these carry a role the token issuance hook wrote into a token GoTrue signed. A rule that is right about a constructed caller and unreachable from a real one passes there and fails here.
> 3. **«An attempt to create an admin» is measured at the wire.** No route takes a role — `POST /v1/providers` passes a constant — so what an attempt can be is a body naming one, and the criterion is met by the 422 that names the `role` field *and* by the address still being free afterwards. The policy that refuses `role='admin'` on the service path is step-6's `TestTheServicePathRefusesToWriteAnAdmin` and is not repeated.
> 4. **The verifier is pointed at the provider's own key set.** Measured on 2026-08-16 against the pinned image: GoTrue serves `/.well-known/jwks.json` carrying the session `kid`, which is the address a deployment derives from the issuer. A fixture publishing the same public key would verify the same tokens and leave the one moving part — the provider publishing the key it signs with — unmeasured.
> 5. **Visibility is re-read over all five relations.** Four are compared as exact sets in one loop, per caller. `care_team_assignments` is among them by decision rather than by symmetry: row level security on a referenced table only ever removes rows from an `EXISTS` subquery, and every predicate that reads the care team carries its own `= app.jwt_subject()`, so a care-team policy widened to `USING (true)` leaves all the derived sets unchanged and nothing would notice a doctor reading the clinic's whole care graph. `user_preferences` is measured on its own because a doctor is stopped there by a missing grant — 000004 grants `SELECT` to the patient, the service path and the administrator and to nobody else — and the witness is that the read fails at all: a policy filters a `SELECT` rather than raising, so a doctor merely un-admitted by a policy would get an empty result that reads exactly like a clinic whose patients have no settings.
> 6. **The clinic the walk builds is the policy suite's crosswise fixture** — two doctors, a patient each — because with one of each every row a doctor could read is one they are entitled to, and «does not see the other one» is a claim about an empty table.
> 7. **The residual property is measured as the provider's own answers.** After the claim the previous access token is answered `403 user_not_found` and the rotated refresh token `400 refresh_token_not_found`, both measured against the pinned image before the test was written. The refresh token is exchanged once *before* the claim, so the half measured afterwards is the one the exchange handed back and the control cannot be mistaken for it. The mutation the test must fail against is a claim that clears the account's confirmation instead of deleting it: plausible, less destructive, enough to make the retry answer 201 — and it leaves the identifier, the session and any password set from it in place. Run: all three assertions go red.
> 8. **`GOTRUE_DISABLE_SIGNUP` gained a name and a pin.** The harness disabling public registration is evidence about the harness until something compares it with the stack this repository ships, so the value is a constant in `testsupport` and a test reads it off the running container and off `docker-compose.yml`.
