...
**Смежное:** у прототипа есть `thigh`, которого нет в §03, и нет `HIP`/`CHEST`,
которые в §03 есть. `HIP` — не переименованное бедро: метрики бедра в модели нет
вовсе. `CHEST` намеренно остаётся непромеренным — критерий «метрика без измерений
говорит об этом» требует метрики, у которой их нет.
